...
from pywise.utils.generator.templates import model_template
        ...
        for field_name, field_schema in properties.items():
            ...
            if field_schema.get("$ref") is not None:
                ...
                if model_class_name == f"{nested_model_name}Model":
                    ...
                    imports.append(
                        f"from pywise.models.manage.{nested_model_name}Model import {nested_model_name}Model"
                    )
            else:
                    ...
                    if schema_type == "array":
                        ...
                        if item_type is None:
                            if field_schema.get("items").get("$ref") is not None:
                                ...
                                imports.append(
                                    f"from pywise.models.manage.{nested_class_name} import {nested_class_name}"
                                )
                            else:
                                field_type = "list[Any]"
                        else:
