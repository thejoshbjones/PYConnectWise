...
from pywise.utils.fs import save_py_file
from pywise.utils.naming import to_title_case_preserve_case
from pywise.utils.generator.model_gen import generate_model
import os
import re
...
def generate_endpoint(
    ...
    for child_endpoint in child_endpoints:
        ...
        if "Id" in child_endpoint:
            ...
            additional_imports.append(f"from pywise.endpoints.manage.{id_child_endpoint_class_name} import {id_child_endpoint_class_name}")
            has_id_child = True
        else:
            ...
            additional_imports.append(f"from pywise.endpoints.manage.{endpoint_class_name.replace('Endpoint', child_endpoint_class_name)} import {endpoint_class_name.replace('Endpoint', child_endpoint_class_name)}")
                ...
                if schema_ref:
                    ...
                    if generated_class_name not in imported_models:
                        additional_imports.append(
                            f"from pywise.models.manage.{generated_class_name} import {generated_class_name}"
                        )
...
def generate_top_level_endpoint(
    ...
    for child_endpoint in child_endpoints:
        ...
        additional_imports.append(
            f"from pywise.endpoints.manage.{endpoint_class_name.replace('Endpoint', child_endpoint_class)} import {endpoint_class_name.replace('Endpoint', child_endpoint_class)}"
        )
