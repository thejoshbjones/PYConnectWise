from pywise.utils.generator.templates import endpoint_template, model_template, top_level_endpoint_template
from pywise.utils.generator.fs import save_py_file
import os
import re
from pywise.models.utils.naming import to_snake_case

def generate_endpoint_legacy(file_output_directory: str, path: str, path_info: dict, relationships: dict, id_relationships: dict, parent_path: str = ''):
    cleaned_path = re.sub(r'/{.*?}', '', path).rstrip('/')
    replaced_id_path = path.replace("{parentId}", "{id}").replace('{grandparentId}', '{id}').replace('{reportName}', '{id}').replace('{externalId}', '{id}').rstrip('/')
    subbed_path = replaced_id_path.replace("{id}", "Id")
    endpoint_class = ''.join((word[:1].upper() + word[1:]) for word in subbed_path.split('/'))
    ...
    for child_endpoint in child_endpoints:
        ...
        child_endpoint_class = ''.join(word.capitalize() for word in child_endpoint.split('/'))
        child_endpoint_class += 'Endpoint'
        additional_imports.append(f"from .{endpoint_class.replace('Endpoint', child_endpoint_class)} import {endpoint_class.replace('Endpoint', child_endpoint_class)}")
    ...
    if cleaned_path in id_relationships and '{id}' not in path:
        ...
        additional_imports.append(f"from .{id_child_endpoint_class} import {id_child_endpoint_class}")
        has_id_child = True
    ...
    save_py_file(os.path.join(file_output_directory, f"{endpoint_class}.py"), endpoint_code)

def generate_model(file_output_directory: str, model_name: str, model_schema: dict, models: dict):
    ...
    if properties is not None:
        for field_name, field_schema in properties.items():
            ...
            if field_schema.get('$ref') is not None:
                ...
                nested_class_name = generate_model(file_output_directory, nested_model_name, nested_model_schema, models)
                ...
                if class_name == f"{nested_model_name}Model":
                    print(f'                Skipping import for {nested_model_name}Model as its ourself <{class_name}>')
                else:
                    imports.append(f'from pywise.models.{nested_model_name}Model import {nested_model_name}Model')
            else:
                ...
                if field_schema.get("enum") is not None:
                    ...
                    for value in enum_values:
                        if value.lower() == 'none' or value.lower() == 'true' or value.lower() == 'false':
                    ...
                    enums.append({
                        'e_name': enum_name,
                        'fields': [{'v_name': value, 'v_value': value, 'v_type': convert_type_name(schema_type)} for value in pp_values],
                    })
                    ...
                    if schema_type == 'array':
                        ...
                        if item_type is None:
                            if field_schema.get('items').get('$ref') is not None:
                                ...
                                imports.append(f'from pywise.models.{nested_class_name} import {nested_class_name}')
                            else:
                                field_type = 'list[Any]'
                        else:
...
def generate_endpoint(endpoint_output_directory: str, model_output_directory: str, path: str, path_info: dict, relationships: dict, models: dict):
    ...
    for child_endpoint in child_endpoints:
        ...
        if 'Id' in child_endpoint or 'ParentId' in child_endpoint or 'GrandparentId' in child_endpoint or 'ReportName' in child_endpoint or 'ExternalId' in child_endpoint:
            type_of_id = 'Id'
            if 'Id' in child_endpoint:
                type_of_id = 'Id'
            ...
            id_child_endpoint_class = endpoint_class.replace('Endpoint', f'{type_of_id}Endpoint')
            if endpoint_class == id_child_endpoint_class:
                # what happened here? the parent is it's own child is it's own parent!?
                continue

            additional_imports.append(f"from pywise.endpoints.{id_child_endpoint_class} import {id_child_endpoint_class}")
            has_id_child = True
        else:
            child_endpoint_path = child_endpoint
            child_endpoint_class = ''.join((word[:1].upper() + word[1:]) for word in child_endpoint.split('/'))
            child_endpoint_class += 'Endpoint'
            if endpoint_class == child_endpoint_class:
                # what happened here? the parent is it's own child is it's own parent!?
                continue
            additional_imports.append(f"from pywise.endpoints.{endpoint_class.replace('Endpoint', child_endpoint_class)} import {endpoint_class.replace('Endpoint', child_endpoint_class)}")
        ...
        for response in operation_responses.values():
            if response.get('content') is None:
                ...
                if schema_ref:
                    ...
                    generated_class_name = generate_model(model_output_directory, model_name, model_schema, models)
                    ...
                    if generated_class_name not in imported_models:
                        ...
                        print(f'                From: pywise.models.{generated_class_name}')
                        additional_imports.append(f'from pywise.models.{generated_class_name} import {generated_class_name}')
    ...
    save_py_file(os.path.join(endpoint_output_directory, f"{endpoint_class}.py"), endpoint_code)
    return is_top_level_endpoint, endpoint_class

def generate_top_level_endpoint(endpoint_output_directory: str, path: str, path_info:dict, relationships: dict):
    #subbed_path = path.replace("{id}", "Id").replace("{parentId}", "ParentId").replace('{grandparentId}', 'GrandparentId').replace('{reportName}', 'ReportName').replace('{externalId}', 'ExternalId').rstrip('/')
    replaced_id_path = path.replace("{parentId}", "{id}").replace('{grandparentId}', '{id}').replace('{reportName}', '{id}').replace('{externalId}', '{id}').rstrip('/')
    subbed_path = replaced_id_path.replace("{id}", "Id")
    endpoint_class = ''.join((word[:1].upper() + word[1:]) for word in subbed_path.split('/'))
